...
const variants = [
  {
    size: "100ml",
    label: "Full Size",
    desc: "The complete ritual. Lasts 2-3 months with daily use.",
    img: "/rose-ras-100ml.jpg",
    waMsg: "Hi Kosha Life! I'd like to order Rose Ras 100ml.",
  },
  {
    size: "30ml",
    label: "Travel Mini",
    desc: "Take it everywhere. Perfect for first-timers.",
    img: "/rose-ras-mini-bottle.jpg",
    waMsg: "Hi Kosha Life! I'd like to order Rose Ras 30ml Mini.",
  },
];

const benefits = [
  { title: "Deep Hydration", desc: "Replenishes moisture at every layer of the skin without heaviness." },
  { title: "pH Balance", desc: "Restores the skin's natural acid mantle after cleansing." },
  { title: "Reduces Redness", desc: "Anti-inflammatory Rosa Damascena calms irritated, reactive skin." },
  { title: "Natural Fragrance", desc: "The scent of real roses — no synthetic fragrance, ever." },
];
...
const photos = [
  "/rose-ras-dark.jpg",
  "/rose-ras-100ml.jpg",
  "/rose-ras-mini-bottle.jpg",
];

export default function RoseRasPage() {
  ...
  return (
    <div style={{ backgroundColor: "#FDFAF4", minHeight: "100vh" }}>
      ...
      >
        ...
        <span className="font-serif italic text-sm" style={{ color: "#C4943A" }}>Rose Ras</span>
        <a
          href={`https://wa.me/919461911119?text=${encodeURIComponent(v.waMsg)}`}
          target="_blank"
      ...
      {/* ── Hero ── */}
      <section className="relative pt-16 grid lg:grid-cols-2 min-h-screen" style={{ backgroundColor: "#2C1A0E" }}>
        {/* Left: video */}
        <div className="relative overflow-hidden" style={{ minHeight: "60vh" }}>
          <video autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover" style={{ filter: "brightness(0.55) saturate(0.9)" }}>
            <source src="/rose-ras-video.mp4" type="video/mp4" />
          </video>
          <div className="absolute inset-0" style={{ background: "linear-gradient(to right, transparent 60%, #2C1A0E 100%)" }} />
        </div>
        ...
        <div className="flex flex-col justify-center px-10 lg:px-16 py-16 relative z-10">
          <motion.p
            ...
            style={{ color: "#C4943A" }}
          >
            Steam Distilled · Rosa Damascena
          </motion.p>
          ...
          <motion.h1
            ...
            style={{ fontSize: "clamp(3rem, 7vw, 5.5rem)", color: "#F2E8D5" }}
          >
            Rose
            <br />
            <em style={{ color: "#C4943A" }}>Ras</em>
          </motion.h1>
          ...
          >
            100% pure steam-distilled rose water from fresh Damask roses. No preservatives. No added fragrance. Just the rose — in its most honest form.
          </motion.p>
          ...
          <motion.div
            ...
            className="mb-10"
          >
            <p className="font-sans text-[9px] tracking-[0.3em] uppercase mb-4" style={{ color: "#C4943A", opacity: 0.7 }}>
              Choose size
            </p>
            <div className="flex gap-4">
              {variants.map((vr, i) => (
                <button
                  ...
                  className="flex-1 py-4 px-5 text-left transition-all duration-300 border"
                  style={{
                    borderColor: activeVariant === i ? "#C4943A" : "rgba(242,232,213,0.2)",
                    backgroundColor: activeVariant === i ? "rgba(196,148,58,0.1)" : "transparent",
                  }}
                >
                  <p className="font-serif text-lg font-light" style={{ color: "#F2E8D5" }}>{vr.size}</p>
                  <p className="font-sans text-[9px] tracking-[0.15em] uppercase mt-1" style={{ color: "#C4943A", opacity: 0.7 }}>{vr.label}</p>
                  <p className="font-sans text-xs font-light mt-2" style={{ color: "#F2E8D5", opacity: 0.55 }}>{vr.desc}</p>
                </button>
              ))}
            </div>
          </motion.div>
          ...
          <motion.a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 px-8 py-4 font-sans text-xs tracking-[0.2em] uppercase w-fit transition-all duration-300 hover:opacity-90"
          ...
          >
            ...
            Order {v.size} on WhatsApp
          </motion.a>
        </div>
      </section>

      {/* ── Photo gallery scroll ── */}
      <section className="py-16 px-6 overflow-hidden" style={{ backgroundColor: "#F2E8D5" }}>
        <p className="font-sans text-[9px] tracking-[0.35em] uppercase mb-8" style={{ color: "#C4943A" }}>Gallery</p>
        <div className="flex gap-5 overflow-x-auto scroll-hide snap-x snap-mandatory pb-2">
          {photos.map((src, i) => (
            <motion.div
              key={i}
              ...
              className="flex-none snap-start relative overflow-hidden"
              style={{ width: "280px", aspectRatio: "3/4" }}
            >
              <Image src={src} alt={`Rose Ras ${i + 1}`} fill className="object-cover" />
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-6" style={{ backgroundColor: "#FDFAF4" }}>
        <div className="max-w-5xl mx-auto">
          <div ref={benefitsRef}>
            ...
            >
              Simple ingredients.<br />Serious results.
            </motion.h2>
          </div>
      ...
      <section className="py-20 px-6" style={{ backgroundColor: "#2C1A0E" }}>
        <div className="max-w-3xl mx-auto" ref={stepsRef}>
          ...
          >
            Morning and night.<br /><em style={{ color: "#C4943A" }}>Or whenever you need to reset.</em>
          </motion.h2>
      ...
      <section className="py-24 px-6 text-center" style={{ backgroundColor: "#F2E8D5" }}>
        ...
        <a
          href={`https://wa.me/919461911119?text=${encodeURIComponent(variants[activeVariant].waMsg)}`}
          target="_blank"
          rel="noopener noreferrer"
